...
function Logged() {
  const BACKEND = 'https://twitter-clone-back.vercel.app';
  ...
  const findTweetFromHashtag = (x) => {
    ...
    fetch(
      `${BACKEND}/users/tweets/hashtag/${hashtagToFind}`,
      {
        method: "GET",
        headers: {
  ...
  const removeTweet = (x) => {
    // confirm(x)
    fetch(`${BACKEND}/users/tweets/delete/${x}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
        ...
        <div className={style.ListTweet}>
          ...
          <div className={style.buttonContainer}>
            <span data-id={data._id} onClick={() => {}} className={style.like}>
              <FontAwesomeIcon style={{height:'20px', width:'20px'}} icon={faHeart}></FontAwesomeIcon>
